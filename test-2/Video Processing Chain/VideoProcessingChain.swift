...
struct VideoProcessingChain {
    ...
    private let actionClassifier = Orthodox_30FPS.shared
...
extension VideoProcessingChain {
    ...
    private func findPosesInFrame(_ frame: CGImage) -> [Pose]? {
        ...
        if let observation = humanBodyPoseRequest.results?.first {
            let a = try! observation.recognizedPoint(.leftWrist)
            print(a.location)
        }
